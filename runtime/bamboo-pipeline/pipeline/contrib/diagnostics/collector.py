...
def collect_runtime_snapshot(root_pipeline_id="", node_id="", process_id=None):
    ...
    if process_id is not None:
        seed_process = PipelineProcess.objects.filter(id=process_id).first()
    ...
    process_queryset = PipelineProcess.objects.all()
    if root_pipeline_id:
    ...
    state_node_ids = _unique(([node_id] if node_id else []) + process_node_ids)
    states = _sorted_states(Status.objects.filter(id__in=state_node_ids)) if state_node_ids else []

    candidate_node_ids = _unique(
        ([node_id] if node_id else [])
        + [getattr(state, "node_id", state.id) for state in states]
        + process_node_ids
    )

    schedule_queryset = ScheduleService.objects.all()
    if process_ids:
    ...
    if candidate_node_ids:
        schedule_queryset = schedule_queryset.filter(activity_id__in=candidate_node_ids)
    else:
